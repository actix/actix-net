...
mod tests {
    ...
    #[tokio::test]
    async fn optional_transform_enabled() {
        let service = Some(AddOne(false))
    ...
    #[tokio::test]
    async fn optional_transform_disabled() {
        let service = None::<AddOne>
            .new_transform(fn_service(|req: u32| async move {
                if req > 10 {
                    Err("call")
                } else {
        ...
        assert_eq!(service.call(1).await, Ok(1));
        assert_eq!(service.call(11).await, Err("call"));
        assert_eq!(poll_fn(|cx| service.poll_ready(cx)).await, Ok(()));
    }
    ...
    struct PendingService(Cell<bool>);
    ...
    #[tokio::test]
    async fn optional_transform_pending_enabled() {
        ...
        let service = true
            ...
            .new_transform(PendingService(Cell::new(true)))
            .await
            .unwrap();
        ...
        assert_eq!(future.await, Ok(6));
    }

    #[tokio::test]
    async fn optional_transform_pending_disabled() {
        let mut constructed = false;
