    ...
    T: AsyncRead + Decoder,
{
    ...
    fn poll_next(self: Pin<&mut Self>, cx: &mut Context<'_>) -> Poll<Option<Self::Item>> {
        ...
        loop {
            ...
            if this.is_readable {
                if this.eof {
                    let frame= this.inner.decode_eof(&mut this.buffer)?;
                    return Poll::Ready(Ok(frame).transpose());
                }

                trace!("attempting to decode a frame");

                if let Some(frame) = this.inner.decode(&mut this.buffer)? {
            ...
            if 0 == ready!( unsafe { Pin::new_unchecked(&mut this.inner)}.poll_read(cx, &mut this.buffer))? {
                this.eof = true;
            }
